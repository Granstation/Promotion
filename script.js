...
const sizeTabBar = document.getElementById("sizeTabBar");
...
const translations = {
  "th": {
    ...
    "washVacuumS": "ล้างรถ ดูดฝุ่น S",
    "washPressureS": "ล้างอัดฉีด S",
    "washEngineS": "ล้างห้องเครื่อง S",
    "washAsphaltS": "ล้างยางมะตอย S",
    "seatCleanS": "ฟอกเบาะ S",
    "carpetCleanS": "ฟอกพรม S",
    "glossS": "เคลือบเงาสี S",
    "ceramicS": "เคลือบแก้วเซรามิก S",
    "polishS": "ขัดเคลือบสี S"
  },
  "en": {
    "title": "Grand Station",
    "total": "Total:",
    ...
    "washVacuumS": "Wash & Vacuum S",
    "washPressureS": "Pressure Wash S",
    "washEngineS": "Engine Bay Cleaning S",
    "washAsphaltS": "Tar Removal S",
    "seatCleanS": "Seat Cleaning S",
    "carpetCleanS": "Carpet Cleaning S",
    "glossS": "Gloss Coating S",
    "ceramicS": "Ceramic Coating S",
    "polishS": "Polish & Wax S"
  }
};
...
function updateSizeVisibility() {
  ...
  if (size === "ALL") {
    ["S", "M", "L", "XL"].forEach(sz => {
      document.querySelectorAll(`label[data-key$="${sz}"]`).forEach(lbl => {
    ...
    ["S", "M", "L", "XL"].forEach(sz => {
      document.querySelectorAll(`label[data-key$="${sz}"]`).forEach(lbl => {
        lbl.style.display = (sz === size) ? "flex" : "none";
      });
    });
  }
}
...
function initUI() {
  ...
  // Size tab bar logic
  sizeTabBar.querySelectorAll(".size-tab").forEach(btn => {
    btn.addEventListener("click", () => {
      sizeTabBar.querySelectorAll(".size-tab").forEach(b => b.classList.remove("active"));
      btn.classList.add("active");
      currentSize = btn.dataset.size;
      updateSizeVisibility();
      // uncheck hidden checkboxes (เฉพาะถ้าไม่ใช่ ALL)
      if (currentSize !== "ALL") {
        ["S", "M", "L", "XL"].forEach(sz => {
          if (sz !== currentSize) {
            document.querySelectorAll(`label[data-key$="${sz}"] input[type="checkbox"]`).forEach(cb => {
              cb.checked = false;
            });
          }
        });
